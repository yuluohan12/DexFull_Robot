...
class G1_29_ArmController:
        ...
        while True:
            ...
            if msg is not None:
                ...
                for id in range(G1_29_Num_Motors):
                    ...
                    lowstate.motor_state[id].vol = getattr(msg.motor_state[id], "vol", 0.0)
    ...
    def _ctrl_motor_state(self):
        ...
        # Diagnostic: track cycle jitter to verify GIL blocking hypothesis
        _diag_interval = 2.0  # log summary every 2s
        _diag_last_log = 0.0
        _diag_max_jitter_ms = 0.0
        _diag_overrun_count = 0
        _diag_loop_count = 0
        _diag_last_loop = time.perf_counter()

        while True:
            start_time = time.time()
            loop_start_perf = time.perf_counter()
            # Measure inter-loop gap (how long since last loop start)
            inter_loop_ms = (loop_start_perf - _diag_last_loop) * 1000.0
            _diag_last_loop = loop_start_perf
            expected_ms = self.control_dt * 1000.0  # 4.0ms for 250Hz
            jitter_ms = abs(inter_loop_ms - expected_ms)
            _diag_max_jitter_ms = max(_diag_max_jitter_ms, jitter_ms)
            _diag_loop_count += 1
            if jitter_ms > expected_ms * 2:  # over 2x expected period
                _diag_overrun_count += 1

            now_ts = time.time()
            if now_ts - _diag_last_log >= _diag_interval:
                # logger_mp.info(
                #     "[G1_29_ArmController] _ctrl_motor_state cycle stats: "
                #     "loops=%d max_jitter_ms=%.1f overruns=%d expected_ms=%.1f",
                #     _diag_loop_count, _diag_max_jitter_ms, _diag_overrun_count, expected_ms,
                # )
                _diag_last_log = now_ts
                _diag_max_jitter_ms = 0.0
                _diag_overrun_count = 0
                _diag_loop_count = 0
