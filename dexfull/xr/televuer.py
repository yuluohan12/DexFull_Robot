...
from dexfull.common.tls import resolve_or_create_tls_pair
...
class TeleVuer:
    def __init__(self, use_hand_tracking: bool, binocular: bool=True, img_shape: tuple=None, display_fps: float=30.0,
        ...
        cert_file, key_file = resolve_or_create_tls_pair(cert_file, key_file)
        ...
        # The control runtime is started with the "spawn" method, and Python
        # propagates spawn as the default for nested multiprocessing.Process.
        # Pickling this bound method would serialize the whole TeleVuer instance,
        # whose Vuer object holds an aiohttp FrozenList that cannot be pickled
        # ("TypeError: self._frozen cannot be converted to a Python object").
        # On POSIX use fork so the already-initialized Vuer + shared memory are
        # inherited in place, matching the original single-process behaviour.
        if os.name == "posix":
            self.process = multiprocessing.get_context("fork").Process(
                target=self._vuer_run
            )
        else:
            self.process = multiprocessing.Process(target=self._vuer_run)
    ...
    @staticmethod
    def _session_upsert(session, *elements, **kwargs):
        """End a stale Vuer handler quietly after its websocket disconnects."""
        try:
            session.upsert(*elements, **kwargs)
            return True
        except AssertionError as exc:
            if "Websocket session is missing" in str(exc):
                return False
            raise
